...
-- this is basically identical to the SMODS luajit libcurl, done so it can access the packaged libcurl dll
-- fucking awful hack though

-- try to get built-in luajit-curl
local succ, curl = require("luajit-curl")

local function libcurl()

	-- return SMODS libcurl if it exists
	if succ then return curl end

	local ffi = require("ffi")
	local succ, curl = pcall(ffi.load, Wormhole.path .. "content/lancer_fan_club/lib/libcurl")
	if not succ then
...
Wormhole.LancerFanClub.libcurl = libcurl